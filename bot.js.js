...
// ── TELEGRAM SETUP ────────────────────────────────────────────────────
const tg = new TelegramBot(process.env.TELEGRAM_BOT_TOKEN, { polling: true });
...
// Отправка сообщения в главный чат (для логов) с меню
async function tgSend(msg) {
  try {
    await tg.sendMessage(TG_CHAT_ID, msg, { parse_mode: 'HTML', disable_web_page_preview: true, ...mainKeyboard });
  } catch(e) {
...
function setupTelegramListeners() {
  ...
  tg.onText(/\/start/, async (msg) => {
    ...
    await tg.sendMessage(chatId, '🤖 <b>BTC 15M Бот</b>\nВыберите действие:', { parse_mode: 'HTML', ...mainKeyboard });
  });
  ...
  tg.onText(/\/balance|💰 Баланс/, async (msg) => {
    ...
    const s = state.stats;
    await tg.sendMessage(msg.chat.id, `<b>Баланс</b>\n💰 Баланс: $${w.balance.toFixed(2)}\n📈 PnL: $${w.pnl.toFixed(2)}\n✅ Побед: ${w.wins}\n❌ Поражений: ${w.losses}\n🔥 Серия: ${s.curStreak}\n🏆 Рекорд: ${s.bestStreak}\n💵 Ставок: ${w.wins+w.losses}`, { parse_mode: 'HTML', ...mainKeyboard });
  });

  // Статус
  tg.onText(/\/status|📊 Статус/, async (msg) => {
    await tg.sendMessage(msg.chat.id, `<b>Статус</b>\n🤖 Бот: ${_userBotOn ? '🟢 Вкл' : '🔴 Выкл'}\n⏰ Расписание: ${isWorkingHours() ? '🟢 Работает' : '🔴 Ночь'}\n📈 Цена BTC: $${Math.round(currentPrice)}\n⏳ До ставки: ${getTimeToNextBet()}\n💰 Баланс: $${state.wallet.balance.toFixed(2)}`, { parse_mode: 'HTML', ...mainKeyboard });
  });

  // Цена
  tg.onText(/\/price|📈 Цена BTC/, async (msg) => {
    await tg.sendMessage(msg.chat.id, `📈 <b>BTC</b>: $${Math.round(currentPrice)}`, { parse_mode: 'HTML', ...mainKeyboard });
  });

  // Время до ставки
  tg.onText(/\/nextbet|⏳ След. ставка/, async (msg) => {
    await tg.sendMessage(msg.chat.id, `⏳ До следующей ставки: <b>${getTimeToNextBet()}</b>`, { parse_mode: 'HTML', ...mainKeyboard });
  });

  // Старт
  tg.onText(/\/startbot|▶ Старт/, async (msg) => {
    if (!isWorkingHours()) {
      await tg.sendMessage(msg.chat.id, '⏰ Нерабочее время. Бот работает 09:00–00:00 МСК.', mainKeyboard);
      return;
    }
    _userBotOn = true;
    await saveState();
    await tg.sendMessage(msg.chat.id, '▶ Бот запущен', mainKeyboard);
  });
  ...
  tg.onText(/\/stopbot|⏹ Стоп/, async (msg) => {
    _userBotOn = false;
    await saveState();
    await tg.sendMessage(msg.chat.id, '■ Бот остановлен', mainKeyboard);
  });
  ...
  tg.onText(/\/reset|🔄 Сброс/, async (msg) => {
    ...
    roundPlaced = null;
    await saveState();
    await tg.sendMessage(msg.chat.id, '↺ Сброс. Баланс: $100', mainKeyboard);
  });
...
/help – Эта справка`;
    await tg.sendMessage(msg.chat.id, helpText, { parse_mode: 'HTML', ...mainKeyboard });
  });

  // Обработка неизвестных сообщений – просто показываем меню
  tg.on('message', (msg) => {
    // не обрабатываем, если это команда
    if (msg.text && msg.text.startsWith('/')) return;
    // Можно ничего не делать или отправить подсказку
  });
}
...
async function start() {
  ...
  connectPriceWS();
  listenForCommands();
  setupTelegramListeners(); // подключаем телеграм-команды
